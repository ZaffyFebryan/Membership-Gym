...
<body class="konfirm">
    ...
    <div class="content">
        <div class="satu">
            <h2>Data Pembayaran<span>
                <a href="profiladmin.php" style="margin-left: 50rem; margin-right: 2rem;">
        ...
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                echo "<table>";
                echo "<tr><th>ID Pembayaran</th><th>ID User</th><th>ID Layanan</th><th>ID Trainer</th><th>Nama Depan</th><th>Nama Belakang</th><th>Layanan</th><th>Tanggal Latihan</th><th>Waktu Latihan</th><th>Trainer</th</tr>";
                $paymentIdCounter = 1;
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    echo "<td class='payment-id'></td>";
                    ...
                    echo "<td><button class='status-button' onclick='confirmPayment(this, $paymentIdCounter)'>Konfirmasi Pembayaran</button></td>";
                    echo "</tr>";
                    $paymentIdCounter++;
                }
                echo "</table>";
    ...
    <script>
        function confirmPayment(button, id) {
            var row = button.parentElement.parentElement;
            var paymentIdCell = row.getElementsByClassName('payment-id')[0];
            paymentIdCell.textContent = 'A' + id;
    
            button.textContent = 'Pembayaran sudah dikonfirmasi';
        ...
        function printData() {
            ...
            printWindow.document.write('<html><head><title>Konfirmasi Pembayaran</title></head><body>');
            printWindow.document.write('<h1>Konfirmasi Pembayaran</h1>');
